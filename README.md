...
| Plaintext vs Encrypted benchmarks | ✅ Done | Paired 30-trial comparison: no detectable latency difference (p=0.299) |
| Scalability sweep | ✅ Done | 10–1000 subscribers; recall 0.98 up to 200, UDP-drop limited beyond |
| Unit tests | ✅ Done | TLV, Geo, Paillier, HEPS, Brake, crypto-microbench, encrypted-cross-broker |
| Integration & Benchmark scripts | ✅ Done | Single-broker, multi-broker, crypto comparison, and dynamics scripts |
...
└── results/
    ...
    └── final/                 # Authoritative final data (30 trials) — see below
```
...
| §4.2 Proximity Evaluation | Stretch, recall, matching cost, traffic ratio | ✅ Final data in `results/final/` |
...
>
> The final, seed-controlled 30-trial sweep now lives in
> `results/final/single_brake_{1,2,4,1000}.csv`, summarised in
> `results/final/summary.txt` (recall 0.452 → 1.000, stretch 2.172 → 1.000).

### Phase 2: Multi-Broker Hierarchical Routing

> ⚠️ **Data withdrawn — superseded by the final 30-trial run.** The previously
> published table (recall 0.94–0.96 across brake limits, 5 trials) predates the
> reproducibility rework and does not match the final, seed-controlled 30-trial
> results. In the final data (three-broker tree, 20 publishers, 50 subscribers)
> recall runs from **0.227 at brake_limit=1 to 0.981 at brake_limit=1000**, with
> stretch 3.331 → 1.013 and traffic ratio 1.087 → 1.168 (Table 2 of the thesis).
> The old 0.94–0.96 figures understated the brake's cost by a large margin and are
> **removed rather than left to mislead.**
>
> The authoritative numbers are in `results/final/multi_brake_{1,2,4,1000}.csv`,
> summarised in `results/final/summary.txt` and `results/final/tables.tex`.

### Phase 3: Privacy Layer Overhead

> ⚠️ **Data withdrawn — superseded by the final paired 30-trial comparison.** The
> previously published table (plaintext 109 ms vs encrypted 358 ms, a "3.3x
> bounded latency overhead", 5 trials) conflicts with the final measurement. The
> final run (single broker, 10 publishers, 50 subscribers, brake_limit=4, 30
> paired trials) finds **no detectable latency difference** between the modes:
> Wilcoxon $p = 0.299$, median paired difference $+0.38$ ms (mean 63.6 ms
> plaintext vs 63.4 ms encrypted). The old 3.3x figure came from the earlier
> unpaired, cold-start measurement with per-call RNG reseeding, and is **removed
> rather than left to mislead.**
>
> The authoritative numbers are in `results/final/plain.csv` and
> `results/final/encrypted.csv`, with the paired comparison in
> `results/final/summary.txt` and `results/final/numbers.tex`.
...
- [x] Unit tests (TLV, Geo, Paillier, HEPS, Brake)
...
- [x] Phase 3 benchmarks: no detectable broker-side latency difference (p=0.299)

### Thesis Write-Up (Weeks 12–14) ✅
- [x] Full dissertation draft
- [ ] HEPS decentralization analysis (HAP/HEP/HIP trust models)
- [x] Real-system vs simulation gap analysis
- [x] Final submission
...
**Validation Status:**
- ✅ "steer users to the nearest copy" → recall 0.983, stretch 1.013 at brake_limit=4 (30 trials)
- ✅ "resolve names privately" → Modified Paillier Match implemented and verified (with the hash-grouping caveat)
- ✅ "bounded, measurable cost" → no detectable broker-side latency difference (p=0.299); Match 0.0127 ms/op, bounded by S×0.0127 ms
